...
{
    public static partial class ShellyDeviceHelperGen1
    {
        #region DataObjects
        ...
        public static void SetupLogin(string ipv4)
        {
            var respo = SetLoginSettings(ipv4);
            if (!respo.enabled)
                throw new ApplicationException("Securing device failed");
        }

        private static ShellyDeviceHelperGen1.GetLoginSettingsResponse SetLoginSettings(string ipv4)
        {
            ...
            using (var cli = new ShellyWebClient())
            {
                ...
                {
                    string json = cli.DownloadString(url);
                    var sdi = JsonConvert.DeserializeObject<ShellyDeviceHelperGen1.GetLoginSettingsResponse>(json);
                    return sdi;
                }
                catch (WebException)
                {
        ...
        public static ShellyDeviceHelperGen1.GetSettingsResponse SetMqtt(string ipv4)
        {
            ...
            using (var cli = new ShellyWebClient())
            {
                ...
                {
                    string json = cli.DownloadString(url);
                    var sdi = JsonConvert.DeserializeObject<ShellyDeviceHelperGen1.GetSettingsResponse>(json);
