...
module HomeAutomation.Me.HomeAutoControlPage {

    export interface IHomeAutoControlPageScope extends ng.IScope {
        events: HomeAutoControlPage;
        view: string;
    ...
    export class HomeAutoControlPage {

        connection: signalR.HubConnection;
        scope: IHomeAutoControlPageScope;
        static $inject = ["$scope"];

        constructor($scope: IHomeAutoControlPageScope) {
            this.scope = $scope;
            var self = this;
        ...
        public makeRoomDeviceTree(sc: IHomeAutoControlPageScope, data: DeviceInfo[]) {
        ...
        private findRoom(rooms: Array<HomeAutoRoomForDevices>, sc: IHomeAutoControlPageScope): HomeAutoRoomForDevices {
            return rooms[0];
        }

        private identifyMainRole(device: HomeAutoDevice, sc: IHomeAutoControlPageScope): string {
        ...
        private fillMainStatus(device: HomeAutoDevice, sc: IHomeAutoControlPageScope): string {
            if (device.deviceRoles != null) {
                for (var i = 0; i < device.deviceRoles.length; i++) {
...
var AuthApp = angular.module('HomeAutoControlPageApp', ['ui.select2', 'ngAnimate']);
AuthApp.controller('HomeAutoControlPageController', HomeAutomation.Me.HomeAutoControlPage.HomeAutoControlPage);
